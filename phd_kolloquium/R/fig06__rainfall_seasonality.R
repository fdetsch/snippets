...
# metoffice data
prcp_kia_moshi <- read.table("data/metoffice_1973-2013.csv", 
                             sep = " ", header = TRUE)
...
data(gsodstations)
dir_gsod <- "data/gsod"
if (!dir.exists(dir_gsod)) dir.create(dir_gsod)
...
# visualize
levels(harm_prcp[, 1]) <- c("Makindu", "Southern Kilimanjaro region")
harm_prcp[, 1] <- factor(harm_prcp[, 1], levels = rev(levels(harm_prcp[, 1])))

cols <- c("black", "grey50")
names(cols) <- c("Southern Kilimanjaro region", "Makindu")
...
p_ssn <- ggplot(aes(x = month, y = x, group = station, colour = station, 
  ...
  theme(legend.position = c(1, 1), legend.justification = c(.95, .75), 
        legend.text = element_text(size = 9), legend.key.size = unit(5, "mm"),
...
## manuscript version
tiff("img/rainfall_seasonality.tiff", width = 10, height = 9, units = "cm", 
