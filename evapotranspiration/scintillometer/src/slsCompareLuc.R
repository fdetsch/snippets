...
foreach(h = list(c("sav0", "mai0"), c("sav5", "mai4"), 
  ...
  png(paste0("scintillometer/out/slsCompareLuc_", 
             paste(h, collapse = "_"), ".png"), 
      width = 25, height = 16, units = "cm", res = 300, pointsize = 16)
  print(ggplot(aes(x = hour, y = waterET, fill = plotid), data = dat) + 
    geom_boxplot(outlier.colour = NA, notch = TRUE) + 
