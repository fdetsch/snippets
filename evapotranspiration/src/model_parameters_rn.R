...
theta.fls.time <- sapply(strsplit(basename(theta.fls), "\\."), "[[", 3)
theta.index.day <- which(as.numeric(theta.fls.time) < 1200)
theta.index.night <- which(as.numeric(theta.fls.time) >= 1200)
...
w.index.day <- which(as.numeric(w.fls.time) < 1200)
w.index.night <- which(as.numeric(w.fls.time) >= 1200)

w.fls.day <- w.fls[w.index.day]

df.w.fls.day <- data.frame(date = w.fls.date[w.index.day], 
