...
scale <- list("SpatialPolygonsRescale", layout.scale.bar(), scale = 0.08998623, 
              offset = c(37.06, -3.38), fill = c("transparent", "black"))
text1 = list("sp.text", c(37.06, -3.355), "0", cex = 1.2, font = 2)
...
p_bing <- spplot(rst_kili[[1]], col.regions = NA, colorkey = FALSE, 
                 sp.layout = list(rgb2spLayout(rst_kili, quantiles = c(0, 1)), 
                                  list("sp.text", loc = c(37.005, -2.84), 
...
p_ioa_gimms <- spplot(rst_ioa_gimms, col.regions = envinmrPalette(500), 
  ...
  latticeExtra::layer(sp.text("b)", loc = c(37.005, -2.84), font = 2, cex = 1.2, 
                              adj = c(.1, 1), col = "black"))
...
source("/media/permanent/phd/egu_2016/poster/R/fig01_homogeneities.R")
...
downViewport(trellis.vpname("figure"))
vp1 <- viewport(x = -.15, y = 0, width = .1, height = .5, 
                just = c("left", "bottom"), name = "vp_key")
pushViewport(vp1)
draw.colorkey(key = list(col = envinmrPalette(500), labels = list(cex = 1.2),
                         width = .6, height = .6,
                         at = seq(.35, .95, .005), 
                         space = "left"), draw = TRUE)

grid.text(expression("IOA"["s"]), x = -.5, y = .5, rot = 90, 
          gp = gpar(font = 2, cex = 1.5))

upViewport(0)
vp2 <- viewport(x = .41, y = .5, width = .6, height = .9, 
                just = c("left", "center"), name = "vp_fig2")
pushViewport(vp2)
