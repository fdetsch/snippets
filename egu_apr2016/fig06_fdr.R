...
ch_dir_outdata <- switch(Sys.info()[["sysname"]], 
                         "Windows" = "D:/kilimanjaro/ndvi_comparison/out/mannkendall", 
                         "Linux" = "/media/fdetsch/XChange/kilimanjaro/ndvi_comparison/out/mannkendall")
  ...
  p_heatmap <- 
    levelplot(rst_fdr, margin = FALSE, col.regions = envinmrPalette(501),
              colorkey = list(space = "top", width = .6, height = .5, 
                              labels = list(cex = 1.2)), 
              main = list("FDR", hjust = -.4, vjust = .1, cex = 1.5), at = seq(0, 1, .002),
              xlab = list("Power", cex = 1.5, vjust = -.5), 
              ylab = list("P", cex = 1.5),
              scales = list(cex = 1.2))
    ...
    levelplot(z ~ x * y, colorkey = FALSE, 
              panel = function(...) {
                panel.smoothconts(zlevs.conts = seq(.1, .9, .1), 
                                  ...
                                  col = "black", cex = 1.8, labcex = 1.2, method = "edge", ...)
              }, scales = list(cex = .7))
  } else {
    levelplot(z ~ x * y, colorkey = FALSE, 
              panel = function(...) {
...
tiff(paste0(dir_img, "figure06.tiff"), width = 25, height = 15, 
